...
namespace DreamWorld.Levels.VillageLevel.Entities
{
    public class Crate : Entity
    {
        ...
        {
            Model = GameScreen.Content.Load<Model>(@"Models\Village\Crate");
            base.LoadContent();
        }
        ...
        protected override void GetPhysicsInformation(out JigLibX.Physics.Body body, out JigLibX.Collision.CollisionSkin skin, out Vector3 centerOfMass)
        {
            ...
            // Primitive:
            box = new JigLibX.Geometry.Box(Vector3.Transform(new Vector3(-6f, 0f, -6f), Matrix.CreateScale(Scale)),
                Matrix.CreateFromQuaternion(new Quaternion(0f, 0f, 0f, 0.9999999f)),
                Vector3.Transform(new Vector3(12f, 12f, 12f), Matrix.CreateScale(Scale)));
